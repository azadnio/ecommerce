...
@Component({
  ...
  imports: [TitleCasePipe, ProductCard, MatSidenav, MatSidenavContainer, MatSidenavContent, MatNavList, MatListItem, RouterLink],
  template: `
        ...
        <div class="p-6">
          ...
          @for(cat of categories(); track cat){
            <mat-list-item class="my-2" [activated]="cat === category()" [routerLink]="['/products', cat]">
              <span matListItemTitle class="font-medium" [class] = "cat === category() ? 'text-white': null">{{cat}}</span>
            </mat-list-item>
          }
        </mat-nav-list>
        </div>

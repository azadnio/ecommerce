...
export class SeoManager {
  ...
  private readonly defaultTitle = 'My E-commerce Store';
  private readonly defaultImage = 'https://dummyimage.com/600x400/000/fff&text=E-commerce+Store';

  updateSeoData(seoData: SeoData) {
    this.title.setTitle(seoData.title || this.defaultTitle);
    ...
    if (this.request) {
      ...
      const host = headers?.get('x-forward-host') || headers?.get('host');
      origin = host ? protocol + host : '';
    }
    else if (typeof window !== 'undefined') {
      origin = window.location.origin;
    }
    const fulUrl = origin + this.router.url;
    ...
    canonicalLink.setAttribute('href', fulUrl);
    ...
    this.meta.updateTag({ property: 'og:title', content: seoData.title || this.defaultTitle });
    this.meta.updateTag({ property: 'og:description', content: seoData.description });
    this.meta.updateTag({ property: 'og:url', content: fulUrl });
